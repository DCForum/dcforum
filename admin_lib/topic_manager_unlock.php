...
function topic_manager_unlock() {
   ...
   if ($in['ssaz'] == 'doit') {
      ...
      foreach ($in['id'] as $id) {
         unlock_topic($in['forum'],$id);
      }

      print "<tr class=\"dclite\"><td 
              class=\"dclite\">The topics you selected have been unlocked. \n";
      print "</td></tr>\n";
      end_table();
   ...
   elseif ($in['ssaz'] == 'list') {
      ...
      elseif ($start_time > $stop_time) {
         ...
         $in['desc'] = "Following is a list of topics that are currently
                     locked and meet your search criteria.  Select
                     topics you wish to unlock and then submit this form.";
