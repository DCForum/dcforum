...
function init()
{
    ...
    if ($in['saz']) {
        ...
        if ($in['az'] != 'general') {
            ...
            if ($in['password_1'] == '')
                $errors[] = "First password field was left blank";

            if ($in['password_2'] == '')
                $errors[] = "Second password field was left blank";

            if ($in['password_1'] != $in['password_2'])
                $errors[] = "Two passwords are not the same";
...
function create_forum_table()
{
    ...
    $q = "CREATE TABLE IF NOT EXISTS " . DB_FORUM . " (
                   ...
                   type                TINYINT UNSIGNED NULL DEFAULT 10,
                   forum_order         TINYINT UNSIGNED NULL DEFAULT 0,
                   name                CHAR(100),
                   description         TEXT,
                   num_topics          INT UNSIGNED NULL DEFAULT 0,
                   num_messages        INT UNSIGNED NULL DEFAULT 0,
                   last_date           TIMESTAMP,
                   last_author         CHAR(30),
                   ...
                   top_template        CHAR(50) NULL DEFAULT 'top.html',
                   bottom_template     CHAR(50) NULL DEFAULT 'bottom.html',
                   PRIMARY KEY (id) ) 
                   AUTO_INCREMENT = 100 ";
    db_query($q);
    ...
    $q = "CREATE TABLE IF NOT EXISTS " . DB_SESSION . " (
          ...
          username  CHAR(30) NOT NULL,
             ...
             email  CHAR(50) NOT NULL,
               ...
               utt  ENUM('yes','no') NULL DEFAULT 'no',
                uu  INT UNSIGNED NOT NULL,
                uv  CHAR(10) NOT NULL,
                ue  ENUM('yes','no') NULL DEFAULT 'no',
                ug  ENUM('yes','no') NULL DEFAULT 'no',
                uh  ENUM('yes','no') NULL DEFAULT 'yes',
                uj  ENUM('yes','no') NULL DEFAULT 'no',
                uw  CHAR(30) NULL DEFAULT 'english',
         time_mark  TEXT NULL,
         last_date  TIMESTAMP,
           PRIMARY KEY (id),
             INDEX s_index_1 (id),
...
function create_search_param_table()
{
    ...
    $q = "CREATE TABLE IF NOT EXISTS " . DB_SEARCH_PARAM . " (
      ...
      recursive_search  ENUM('Yes','No') NULL DEFAULT 'No',
          search_field  CHAR(20) NULL DEFAULT 'subject_message',
...
function create_faq_table()
{
    ...
    if (is_empty_table(DB_FAQ_TYPE)) {
        ...
        $q = "INSERT INTO " . DB_FAQ_TYPE . "
                 VALUES (null,'General FAQ'),
                        (null,'Registration FAQ'),
                        (null,'Icons FAQ'),
                        (null,'User How-to FAQ'),
                        (null,'User Functions FAQ') ";
        db_query($q);
        ...
        foreach ($help as $type => $array) {
            ...
            foreach ($help[$type] as $question => $answer) {
                ...
                $q = "INSERT INTO " . DB_FAQ . "
                       VALUES (null,'$type','$question','$answer') ";
...
function create_user_table()
{
    ...
    $q = "CREATE TABLE IF NOT EXISTS " . DB_USER . " (
         ...
         reg_date   TIMESTAMP,
         last_date  TIMESTAMP,
         num_posts  INT UNSIGNED NULL DEFAULT 0,
         num_votes  TINYINT UNSIGNED NULL DEFAULT 0,
         points     TINYINT UNSIGNED NULL DEFAULT 0,
         pa         CHAR(30) NULL DEFAULT '',
         pb         CHAR(30) NULL DEFAULT '',
         pc         CHAR(100) NULL DEFAULT '',
         pd         ENUM('male','female') DEFAULT 'male',
         pe         CHAR(30) NULL DEFAULT '',
         pf         CHAR(20) NULL DEFAULT '',
         pg         CHAR(30) NULL DEFAULT '',
         ph         CHAR(100) NULL DEFAULT '',
         pi         CHAR(100) NULL DEFAULT '',
         pj         CHAR(100) NULL DEFAULT '',
         pk         CHAR(255) NULL DEFAULT '',
         pl         CHAR(100) NULL DEFAULT '',
         pm         CHAR(100) NULL DEFAULT '',
         pn         CHAR(100) NULL DEFAULT '',
         po         CHAR(100) NULL DEFAULT '',
         uw         CHAR(30) NULL DEFAULT '',
         ut         TINYINT UNSIGNED NULL DEFAULT '1',
         utt         ENUM('yes','no') NULL DEFAULT 'no',
         uu         INT UNSIGNED NULL DEFAULT '30',
         uv         CHAR(10) NULL DEFAULT 'dcf',
         ua         ENUM('yes','no') NULL DEFAULT 'no',
         ub         ENUM('yes','no') NULL DEFAULT 'yes',
         uc         ENUM('yes','no') NULL DEFAULT 'yes',
         ud         ENUM('yes','no') NULL DEFAULT 'yes',
         ue         ENUM('yes','no') NULL DEFAULT 'no',
         uf         ENUM('yes','no') NULL DEFAULT 'yes',
         ug         ENUM('yes','no') NULL DEFAULT 'yes',
         uh         ENUM('yes','no') NULL DEFAULT 'yes',
         ui         ENUM('yes','no') NULL DEFAULT 'yes',
         uj         ENUM('yes','no') NULL DEFAULT 'no',
         PRIMARY KEY (id),
         INDEX index_1 (id) )
         AUTO_INCREMENT = 99999";
    ...
    foreach ($setup_var_types as $key => $value) {
        ...
        if ($key_array[$key] != 1) {
            ...
            $q = "INSERT INTO " . DB_SETUP . "
                 VALUES (null,'$key','$default','{$setting_type['$value']}') ";
            db_query($q);
        }
    ...
    if (is_empty_table(DB_NOTICE)) {
        ...
        foreach ($notice_vars as $key => $value) {
            ...
            $q = "INSERT INTO " . DB_NOTICE . "
               VALUES (null,'$key','$subject','$desc','$message') ";
            db_query($q);
        }
    }
}
...
function create_task_table()
{
    ...
    if (is_empty_table(DB_TASK)) {
        $q = "INSERT INTO " . DB_TASK . "
               VALUES (null,'subscription',NOW()) ";
        db_query($q);
        $q = "INSERT INTO " . DB_TASK . "
               VALUES (null,'optimize',NOW()) ";
        db_query($q);
        $q = "INSERT INTO " . DB_TASK . "
               VALUES (null,'backup',NOW()) ";
        db_query($q);
    }
...
function create_event_table()
{
    ...
    $q = "CREATE TABLE IF NOT EXISTS " . DB_EVENT_REPEAT . " (
            id    INT UNSIGNED AUTO_INCREMENT NOT NULL ,
            type  ENUM('0','1','2') NULL DEFAULT '0',
