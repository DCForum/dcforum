...
function registration_form() {
  ...
  // Ok, begin form
   ...
   while(list($key,$val_array) = each($param_login)) {
      ...
      if ($key == 'password' and 
        ...
        // do nothing
         ...
         $fields = split('[\|]',$param_login[$key]['form']);
   ...
   if ($in['this'] == DCA) {
      $form = form_element("status","radio",array("on","off"),"");
...
function user_account_form() {
  ...
  // Ok, begin form
   ...
   if ($in['this'] == DCA) {
      $form = form_element("status","radio",array("on","off"),"$in[status]");
